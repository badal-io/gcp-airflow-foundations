...
class TestMigrateSchema(unittest.TestCase):
    ...
    @mock.patch('airflow.providers.google.cloud.operators.bigquery.BigQueryHook.get_schema')
    @mock.patch('airflow.providers.google.cloud.operators.bigquery.BigQueryHook.run_query')
    @mock.patch('airflow.providers.google.cloud.operators.bigquery.BigQueryHook.update_table_schema')
    def test_execute(self, mock_bq_update_table_schema, mock_bq_run_query, mock_bq_get_schema, mock_insert_change_log_rows):
        ...
        mock_insert_change_log_rows.assert_called_once_with(
            [{'table_id': 'test-table-id', 'dataset_id': 'test-dataset', 'column_name': 'column', 'type_of_change': 'column addition'}]
        )
...
class TestSchemaMigrationAudit(unittest.TestCase):
    ...
    def test_execute(self, mock_create_empty_table, mock_bq_client):
        ...
        migration_audit.insert_change_log_rows(
            change_log=[{'table_id': 'test-table-id', 'dataset_id': 'test-dataset', 'column_name': 'column', 'type_of_change': 'column addition'}]
        )
