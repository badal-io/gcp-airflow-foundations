...
class TestHdsMergeSCD2(object):
    ...
    def setup(
        ...
        self.mock_new_data_rows = [
            {"customerID": "customer_1", "key_id": 1, "city_name": "Ottawa"},
            {"customerID": "customer_4", "key_id": 4, "city_name": "Montreal"},
        ]
    ...
    def clean_up(self):
        ...
        target_table_ref = self.client.dataset(self.target_dataset).table(
            f"{self.target_table_id}_HDS_SCD2"
        )
    ...
    def test_merge_hds_with_deletion(self):
        ...
        af_metadata_expired_at = next(
            (i["af_metadata_expired_at"] for i in query_results if i["key_id"] == 1),
            None,
        )
