...
def create_truncate_sql(
    ...
    return f"""
            ...
            SELECT {columns_str_source}, CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP(), TO_BASE64(MD5(TO_JSON_STRING(S))), TO_BASE64(MD5(ARRAY_TO_STRING([{",".join(["CAST(S.`{}` AS STRING)".format(surrogate_key) for surrogate_key in surrogate_keys])}], "")))
    ...
    if column_mapping:
        ...
        return f"""
            ...
            [f'T.{column_mapping[surrogate_key]}=S.{surrogate_key}' for surrogate_key in surrogate_keys])}
                ...
                VALUES ({columns_str_source}, CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP(), TO_BASE64(MD5(TO_JSON_STRING(S))), TO_BASE64(MD5(ARRAY_TO_STRING([{",".join(["CAST(S.`{}` AS STRING)".format(surrogate_key) for surrogate_key in surrogate_keys])}], ""))))
            """
    else:
        columns_str_target = columns_str_source
        return f"""
...
def create_scd2_sql_with_hash(
    ...
    hash_column_name = hds_metadata.hash_column_name
    status_column_name = hds_metadata.status_column_name
    eff_start_time_column_name = hds_metadata.eff_start_time_column_name
    eff_end_time_column_name = hds_metadata.eff_end_time_column_name
    ...
    if column_mapping:
        ...
        return f"""
                ...
                USING (
                    ...
                    SELECT
                        ...
                        WHERE ( 
                                ...
                                AND target.{eff_end_time_column_name} IS NULL)) S
                ...
                WHEN MATCHED AND MD5(ARRAY_TO_STRING([{",".join(["CAST(T.`{}` AS STRING)".format(column_mapping[i]) for i in columns])}], "")) != MD5(ARRAY_TO_STRING([{",".join(["CAST(S.`{}` AS STRING)".format(col) for col in columns])}], "")) THEN UPDATE
                    SET {eff_end_time_column_name} = CURRENT_TIMESTAMP(), is_current = FALSE
                WHEN NOT MATCHED THEN
                    INSERT ({columns_str_target}, {eff_start_time_column_name}, {eff_end_time_column_name}, {status_column_name}, {hash_column_name})
                    VALUES ({",".join(["join_key_{}".format(surrogate_key, surrogate_key) for surrogate_key in surrogate_keys])},{",".join(["`{}`".format(col) for col in columns if col not in surrogate_keys])}, CURRENT_TIMESTAMP(), NULL, TRUE, TO_BASE64(MD5(TO_JSON_STRING(S))))
                """
    else:
        columns_str_target = columns_str_source
...
def create_snapshot_sql_with_hash(
    source_dataset: str,
    target_dataset: str,
    source: str,
    target: str,
    ...
    return f"""
            INSERT INTO `{target_dataset}.{target}`
            ({columns_str_target}, {eff_start_time_column_name}, {hash_column_name})
            SELECT {columns_str_source}, CURRENT_TIMESTAMP(), TO_BASE64(MD5(TO_JSON_STRING(S)))
            FROM `{source_dataset}.{source}` S
        """