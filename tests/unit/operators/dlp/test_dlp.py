...
EXPECTED_JOB = {
    'storage_config':{
        'big_query_options':{
            'table_reference': {'project_id': 'test-project', 'dataset_id': 'test-dataset', 'table_id': 'test-table-id'},
            'rows_limit_percent': 10, 'sample_method': 'RANDOM_START'}
    },
...
class TestDlpSubTask(unittest.TestCase):
    ...
    @mock.patch('gcp_airflow_foundations.operators.gcp.dlp.get_dlp_bq_inspection_results_operator.BigQueryHook')
    ...
    def test_dlp_args(self, mock_dlp_hook, mock_bq_hook, mock_bq_sensor_hook, mock_bq_sensor_dlp):
      ...
      #  assert dlp_taskgroup.group_id == "dlp_policy_tags"
        assert taskgroup.children.keys() == {'dlp_policy_tags.delete_old_dlp_results_ods',
                                             'dlp_policy_tags.scan_table_ods',
                                             'dlp_policy_tags.read_dlp_results_ods',
                                             'dlp_policy_tags.update_bq_policy_tags_ods'}

        delete_old_dlp_results_task = taskgroup.children["dlp_policy_tags.delete_old_dlp_results_ods"]
        ...
        update_bq_policy_tags_ods = taskgroup.children["dlp_policy_tags.update_bq_policy_tags_ods"]
